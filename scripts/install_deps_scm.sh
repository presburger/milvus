...
function install_linux_deps() {
  if [[ -x "$(command -v apt)" ]]; then
    ...
    apt install -y wget curl ca-certificates gnupg2  \
      ...
      pkg-config uuid-dev libaio-dev libopenblas-dev libgoogle-perftools-dev liblapacke-dev
    ...
    pip3 install conan==1.64.1 --break-system-packages
    echo "source scl_source enable devtoolset-11" | tee -a /etc/profile.d/devtoolset-11.sh
    echo "source scl_source enable llvm-toolset-11.0" | tee -a /etc/profile.d/llvm-toolset-11.sh
    echo "export CLANG_TOOLS_PATH=/opt/rh/llvm-toolset-11.0/root/usr/bin" | tee -a /etc/profile.d/llvm-toolset-11.sh
  ...
  if [ ! $cmake_version ] || [ `expr $cmake_version \>= 3.26` -eq 0 ]; then
    ...
    wget -qO- "https://cmake.org/files/v3.26/cmake-3.26.5-linux-$(uname -m).tar.gz" | tar --strip-components=1 -xz -C /usr/local
  else
...
function install_mac_deps() {
  ...
  ln -s "$(brew --prefix llvm@15)" "/usr/local/opt/llvm"
  # install rust
  if command -v cargo >/dev/null 2>&1; then
      echo "cargo exists"
      rustup install 1.83
