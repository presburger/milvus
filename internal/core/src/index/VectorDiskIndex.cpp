...
VectorDiskAnnIndex<T>::BuildWithDataset(const DatasetPtr& dataset, const Config& config) {
    ...
    build_config[DISK_ANN_THREADS_NUM] = int(num_threads.value());
...
VectorDiskAnnIndex<T>::update_load_json(const Config& config) {
    ...
    // set threads number
    auto num_threads = GetValueFromConfig<uint32_t>(load_config, DISK_ANN_LOAD_THREAD_NUM);
    AssertInfo(num_threads.has_value(), "param " + std::string(DISK_ANN_LOAD_THREAD_NUM) + "is empty");
    load_config[DISK_ANN_THREADS_NUM] = int(num_threads.value());

    // update search_beamwidth
    auto beamwidth = GetValueFromConfig<uint32_t>(load_config, DISK_ANN_QUERY_BEAMWIDTH);
