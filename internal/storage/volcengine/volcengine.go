...
func (c *CredentialProvider) Retrieve() (minioCred.Value, error) {
	...
	if err != nil {
		metrics.PersistentDataSTSAssumeCounter.WithLabelValues(metrics.FailLabel).Inc()
		return ret, errors.Wrap(err, "failed to retrieve credential from volcengine volcengine")
	}

	if c.akCache != cred.AccessKeyID {
		metrics.PersistentDataSTSAssumeCounter.WithLabelValues(metrics.SuccessLabel).Inc()
	}
